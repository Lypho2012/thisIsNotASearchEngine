...
def create_point_cloud_combined(png, foreimage, backimage):
    img1 = Image.open(f"backend/point_cloud_images/{png}")
    img2 = Image.open(f"backend/point_cloud_images/{foreimage}")
    img3 = Image.open(f"backend/point_cloud_images/{backimage}")
    img2 = img2.resize((img2.width,img1.height))
    img3 = img3.resize((img3.width,img1.height))

    points = [(x,y,random.randint(0,img2.width)) for y in range(0,img1.height,RESOLUTION_STEP) for x in range(0,img1.width,RESOLUTION_STEP)]
    ...
    for y in range(0,img2.height,RESOLUTION_STEP): 
        for x in range(0,img2.width,RESOLUTION_STEP):
            if len(img2.getpixel((x,y))) > 3 and img2.getpixel((x,y))[3] == 0:
                points.append((random.randint(0,img1.width),y,x))
                colors.append(img3.getpixel((x,y))[:3])
            else:
                if (x,y) in min_depths:
                    points.append((random.randint(0,min_depths[(x,y)]),y,x))
                else:
                    points.append((random.randint(0,img1.width),y,x))
                colors.append(img2.getpixel((x,y))[:3])
    plot(points,colors)

create_point_cloud_combined("happyfathersdaytransparent.png","dad_foreground.png","dad_background.png")
